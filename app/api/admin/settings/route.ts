...
import {
  ...
  saveAppTheme, saveAppMetrics, saveLogoScale, saveShowTodayDefault, saveQmsSourceMode,
  MODES, QMS_SOURCE_MODES, LOGO_SCALE_MIN, LOGO_SCALE_MAX, type Mode, type QmsSourceMode,
} from "@/lib/settings";
...
const Schema = z.object({
  ...
  slaWaitMinutes: z.coerce.number().int().min(1).max(600).optional(),
  slaServiceMinutes: z.coerce.number().int().min(1).max(600).optional(),
  exceptionMinutes: z.coerce.number().int().min(1).max(1440).optional(),
  showTodayDefault: z.boolean().optional(),
  qmsSourceMode: z.enum(QMS_SOURCE_MODES as unknown as [string, ...string[]]).optional(),
});

export async function GET() {
  const user = await getUser();
  if (!user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  return NextResponse.json({ theme: await getAppTheme(), metrics: await getAppMetrics(), logoScale: await getLogoScale(), showTodayDefault: await getShowTodayDefault(), qmsSourceMode: await getQmsSourceMode(), hasLogo: hasLogo() });
}
...
export async function POST(req: Request) {
  ...
  const { mode, primary, secondary, accent, logo, logoScale, slaWaitMinutes, slaServiceMinutes, exceptionMinutes, showTodayDefault, qmsSourceMode } = parsed.data;
  ...
  if (slaWaitMinutes !== undefined || slaServiceMinutes !== undefined || exceptionMinutes !== undefined) {
    await saveAppMetrics({ slaWaitMinutes, slaServiceMinutes, exceptionMinutes });
  }
  ...
  if (qmsSourceMode !== undefined) {
    await saveQmsSourceMode(qmsSourceMode as QmsSourceMode);
  }
  ...
  await auditFromRequest(req, user.id, "SETTINGS_CHANGE", "app-settings", {
    ...
    slaWaitMinutes, slaServiceMinutes, exceptionMinutes, showTodayDefault, qmsSourceMode,
  });
  ...
  return NextResponse.json({
    ...
    qmsSourceMode: await getQmsSourceMode(),
    hasLogo: hasLogo(),
  });
}
